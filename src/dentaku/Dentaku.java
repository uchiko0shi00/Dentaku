...
public class Dentaku extends JFrame {
    ...
    public Dentaku() {
        ...
        keyPanel.setLayout(new GridLayout(4, 5)); //5行4列のGridLayoutにする
        ...
        keyPanel.add(new SignChange(), 3);
        keyPanel.add(new ClearButton(), 4);
        keyPanel.add(new NumberButton("4"), 5);
        keyPanel.add(new NumberButton("5"), 6);
        keyPanel.add(new NumberButton("6"), 7);
        keyPanel.add(new CalcButton("×"), 8);
        keyPanel.add(new CalcButton("÷"), 9);
        keyPanel.add(new NumberButton("1"), 10);
        keyPanel.add(new NumberButton("2"), 11);
        keyPanel.add(new NumberButton("3"), 12);
        keyPanel.add(new CalcButton("＋"), 13);
        keyPanel.add(new CalcButton("ー"), 14);
        keyPanel.add(new NumberButton("0"), 15);
        keyPanel.add(new NumberButton("00"), 16);
    ...
    public class CalcButton extends JButton implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            double resultValue = (Double.valueOf(result.getText()))//テキストフィールドに表示されているtextを数字に
                    .doubleValue();
            if (isStacked) {
                //以前に演算子ボタンが押されたのなら計算結果を出す
                if (currentOp.equals("＋")) //演算子に応じて計算する
                    stackedValue += resultValue;
                else if (currentOp.equals("ー"))
                    stackedValue -= resultValue;
                else if (currentOp.equals("×"))
                    stackedValue *= resultValue;
                else if (currentOp.equals("÷"))
                    stackedValue /= resultValue;
                result.setText(String.valueOf(Format.format(stackedValue))); //計算結果をテキストフィールドに設定
            }
    ...
    //反転ボタン
    public class SignChange extends JButton implements ActionListener {
        private static final long serialVersionUID = 1L;
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            result.setText(String.valueOf(Format.format(resultValue * (-1))));
